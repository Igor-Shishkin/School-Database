...
public class GeneratePupilData {
    ...
    public Parent generateParent(String surname, char gender, Address address) {
        ...
        int pesel3And4thDigits = setPesel3And4thDigits(year, month);
    ...
    Marks chooseMarks(int grade) {
        if (grade>6) {
            return generateMarks7and8();
        }
        return (grade>3) ? generateMarks456() : null;
    }

    int setPesel3And4thDigits (int year, int month) {
        if (year<2000) {
            return month;
        }
        return (year<2100) ? month +20 : month + 40;
    }
